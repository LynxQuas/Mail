...
function clickHandler(email_id) { 
  ...
  .then(email => {
    ...
    <div>
        <button class="btn btn-danger" id="btn_archived">${email.archived ? "Unarchive" : "Archived"}</button>
...
function reply_click_handler(email) {
  ...
  const first_two_text = responseEmail.subject.slice(0,3).toLowerCase();

  subject.value = first_two_text === prefill_subject.toLowerCase() ? responseEmail.subject : `${prefill_subject} ${responseEmail.subject}`
